...
4. Ingrese su token API de Cloudflare.
   ...
   `CF_AIG_TOKEN` también se acepta como alias para `CLOUDFLARE_API_TOKEN`.
...
2. Establezca las variables de entorno requeridas:
   - ID del proyecto (elija uno):
     - `GOOGLE_CLOUD_PROJECT`
     - `GCP_PROJECT`
     - `GCLOUD_PROJECT`
   - Ubicación (opcional, se verifica en este orden):
     - `GOOGLE_VERTEX_LOCATION`
     - `GOOGLE_CLOUD_LOCATION`
     - `VERTEX_LOCATION`
     - Por defecto es `us-central1` si no se configura ninguno
...
:::tip
La región `global` mejora la disponibilidad y reduce los errores sin costo adicional. Úsela configurando `VERTEX_LOCATION=global`. Utilice puntos finales regionales (por ejemplo, `us-central1`) para los requisitos de residencia de datos. [Más información](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#regional_and_global_endpoints)
:::
