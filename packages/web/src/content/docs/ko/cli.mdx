...
| 변수                                    | 타입      | 설명                                        |
| ------------------------------------- | ------- | ----------------------------------------- |
| `OPENCODE_AUTH_CONTENT`               | string  | 인라인 JSON 인증 자격 증명                         |
...
| `OPENCODE_DISABLE_EXTERNAL_SKILLS`    | boolean | npm 패키지에서 스킬 로드 비활성화                      |
...
| `OPENCODE_DISABLE_PROJECT_CONFIG`     | boolean | 프로젝트 수준 설정 로드 비활성화                        |
...
| `OPENCODE_MODELS_PATH`                | string  | 모델 JSON 캐시 파일의 사용자 지정 경로                  |
...
| `EXA_API_KEY`                         | string  | Exa 웹 검색 API 키 (`OPENCODE_ENABLE_EXA` 필요) |
| `OTEL_EXPORTER_OTLP_ENDPOINT`         | string  | OpenTelemetry OTLP 내보내기 엔드포인트             |
| `OTEL_EXPORTER_OTLP_HEADERS`          | string  | OpenTelemetry OTLP 내보내기 헤더                |
| `OTEL_RESOURCE_ATTRIBUTES`            | string  | OpenTelemetry 리소스 속성 (key=value 쌍)        |
...
| `OPENCODE_EXPERIMENTAL_HTTPAPI`                 | boolean | Effect HttpApi 브릿지 활성화   |
...
| `OPENCODE_EXPERIMENTAL_WORKSPACES`              | boolean | 실험적 워크스페이스 활성화           |
