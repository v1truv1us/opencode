...
4. 输入你的 Cloudflare API Token。
   ...
   `CF_AIG_TOKEN` 也可作为 `CLOUDFLARE_API_TOKEN` 的别名使用。
...
:::tip
`global` 区域可以提高可用性并减少错误，且不会产生额外费用。通过设置 `VERTEX_LOCATION=global` 来使用它。如果有数据驻留需求，请使用区域端点（例如 `us-central1`）。[了解更多](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#regional_and_global_endpoints)
:::
