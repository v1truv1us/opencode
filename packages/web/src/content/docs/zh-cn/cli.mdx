...
| 变量                                    | 类型      | 描述                                         |
| ------------------------------------- | ------- | ------------------------------------------ |
| `OPENCODE_AUTH_CONTENT`               | string  | 内联 JSON 认证凭据                               |
...
| `OPENCODE_DISABLE_EXTERNAL_SKILLS`    | boolean | 禁用从 npm 包加载技能                              |
...
| `OPENCODE_DISABLE_PROJECT_CONFIG`     | boolean | 禁用加载项目级配置                                  |
...
| `OPENCODE_MODELS_PATH`                | string  | 模型 JSON 缓存文件的自定义路径                         |
...
| `EXA_API_KEY`                         | string  | Exa 网络搜索的 API 密钥（需要 `OPENCODE_ENABLE_EXA`） |
| `OTEL_EXPORTER_OTLP_ENDPOINT`         | string  | OpenTelemetry OTLP 导出端点                    |
| `OTEL_EXPORTER_OTLP_HEADERS`          | string  | OpenTelemetry OTLP 导出请求头                   |
| `OTEL_RESOURCE_ATTRIBUTES`            | string  | OpenTelemetry 资源属性（键=值对）                   |
...
| `OPENCODE_EXPERIMENTAL_HTTPAPI`                 | boolean | 启用 Effect HttpApi 桥接 |
...
| `OPENCODE_EXPERIMENTAL_WORKSPACES`              | boolean | 启用实验性工作区             |
