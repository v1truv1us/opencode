...
| 變數                                    | 類型      | 說明                                         |
| ------------------------------------- | ------- | ------------------------------------------ |
| `OPENCODE_AUTH_CONTENT`               | string  | 內嵌 JSON 驗證憑證                               |
...
| `OPENCODE_DISABLE_EXTERNAL_SKILLS`    | boolean | 停用從 npm 套件載入技能                             |
| `OPENCODE_DISABLE_LSP_DOWNLOAD`       | boolean | 停用 LSP 伺服器自動下載                             |
| `OPENCODE_DISABLE_MODELS_FETCH`       | boolean | 停用從遠端來源擷取模型                                |
| `OPENCODE_DISABLE_MOUSE`              | boolean | 停用 TUI 中的滑鼠擷取                              |
| `OPENCODE_DISABLE_PRUNE`              | boolean | 停用舊資料清理                                    |
| `OPENCODE_DISABLE_PROJECT_CONFIG`     | boolean | 停用載入專案層級設定                                 |
...
| `OPENCODE_MODELS_PATH`                | string  | 模型 JSON 快取檔案的自訂路徑                          |
...
| `EXA_API_KEY`                         | string  | Exa 網路搜尋的 API 金鑰（需要 `OPENCODE_ENABLE_EXA`） |
| `OTEL_EXPORTER_OTLP_ENDPOINT`         | string  | OpenTelemetry OTLP 匯出端點                    |
| `OTEL_EXPORTER_OTLP_HEADERS`          | string  | OpenTelemetry OTLP 匯出標頭                    |
| `OTEL_RESOURCE_ATTRIBUTES`            | string  | OpenTelemetry 資源屬性（鍵=值對）                   |
...
| `OPENCODE_EXPERIMENTAL_HTTPAPI`                 | boolean | 啟用 Effect HttpApi 橋接 |
...
| `OPENCODE_EXPERIMENTAL_WORKSPACES`              | boolean | 啟用實驗性工作區             |
