...
4. Geben Sie Ihr Cloudflare-API-Token ein.
   ...
   `CF_AIG_TOKEN` wird auch als Alias für `CLOUDFLARE_API_TOKEN` akzeptiert.
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   - Projekt-ID (eine davon):
     - `GOOGLE_CLOUD_PROJECT`
     - `GCP_PROJECT`
     - `GCLOUD_PROJECT`
   - Standort (optional, in dieser Reihenfolge geprüft):
     - `GOOGLE_VERTEX_LOCATION`
     - `GOOGLE_CLOUD_LOCATION`
     - `VERTEX_LOCATION`
     - Standardmäßig `us-central1`, falls keine gesetzt ist
...
:::tip
Die Region `global` verbessert die Verfügbarkeit und reduziert Fehler ohne zusätzliche Kosten. Verwenden Sie sie durch Setzen von `VERTEX_LOCATION=global`. Verwenden Sie regionale Endpunkte (z. B. `us-central1`) für Datenresidenzanforderungen. [Learn more](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#regional_and_global_endpoints)
:::
