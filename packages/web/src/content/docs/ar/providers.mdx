...
4. أدخل Cloudflare API token.
   ...
   `CF_AIG_TOKEN` مقبول أيضاً كاسم بديل لـ `CLOUDFLARE_API_TOKEN`.
...
2. عيّن متغيرات البيئة المطلوبة:
   - معرّف المشروع (اختر واحداً):
     - `GOOGLE_CLOUD_PROJECT`
     - `GCP_PROJECT`
     - `GCLOUD_PROJECT`
   - الموقع (اختياري، يتم التحقق بهذا الترتيب):
     - `GOOGLE_VERTEX_LOCATION`
     - `GOOGLE_CLOUD_LOCATION`
     - `VERTEX_LOCATION`
     - الافتراضي هو `us-central1` إذا لم يتم تعيين أي منها
   - المصادقة (اختر واحدا):
...
:::tip
تساعد منطقة `global` على تحسين التوفر وتقليل الأخطاء دون تكلفة إضافية. استخدمها عن طريق تعيين `VERTEX_LOCATION=global`. استخدم نقاط نهاية إقليمية (مثل `us-central1`) لمتطلبات موضع البيانات. [اعرف المزيد](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#regional_and_global_endpoints)
:::
