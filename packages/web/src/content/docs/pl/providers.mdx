...
4. Wprowadź swój token API Cloudflare.
   ...
   `CF_AIG_TOKEN` jest również akceptowany jako alias dla `CLOUDFLARE_API_TOKEN`.
...
2. Ustaw wymagane zmienne środowiskowe:
   - Identyfikator projektu (wybierz jeden):
     - `GOOGLE_CLOUD_PROJECT`
     - `GCP_PROJECT`
     - `GCLOUD_PROJECT`
   - Lokalizacja (opcjonalnie, sprawdzana w tej kolejności):
     - `GOOGLE_VERTEX_LOCATION`
     - `GOOGLE_CLOUD_LOCATION`
     - `VERTEX_LOCATION`
     - Domyślnie `us-central1`, jeśli żadna nie jest ustawiona
...
:::tip
Region `global` poprawia dostępność i zmniejsza liczbę błędów bez dodatkowych kosztów. Użyj go, ustawiając `VERTEX_LOCATION=global`. Użyj regionalnych punktów końcowych (np. `us-central1`) w celu spełnienia wymagań dotyczących miejsca przechowywania danych. [Dowiedz się więcej](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#regional_and_global_endpoints)
:::
