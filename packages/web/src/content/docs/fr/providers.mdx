...
`CF_AIG_TOKEN` est également accepté comme alias pour `CLOUDFLARE_API_TOKEN`.
...
2. Définissez les variables d'environnement requises :
   - ID du projet (au choix) :
     - `GOOGLE_CLOUD_PROJECT`
     - `GCP_PROJECT`
     - `GCLOUD_PROJECT`
   - Emplacement (facultatif, vérifié dans cet ordre) :
     - `GOOGLE_VERTEX_LOCATION`
     - `GOOGLE_CLOUD_LOCATION`
     - `VERTEX_LOCATION`
     - Par défaut `us-central1` si aucun n'est défini
...
:::tip
La région `global` améliore la disponibilité et réduit les erreurs sans frais supplémentaires. Utilisez-la en définissant `VERTEX_LOCATION=global`. Utilisez des points de terminaison régionaux (par exemple, `us-central1`) pour les exigences de résidence des données. [En savoir plus](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#regional_and_global_endpoints)
:::
