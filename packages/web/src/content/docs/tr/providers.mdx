...
4. Cloudflare API tokenınızı girin.
   ...
   `CF_AIG_TOKEN`, `CLOUDFLARE_API_TOKEN` için bir takma ad olarak da kabul edilir.
...
2. Gerekli ortam değişkenlerini ayarlayın:
   - Proje kimliği (birini seçin):
     - `GOOGLE_CLOUD_PROJECT`
     - `GCP_PROJECT`
     - `GCLOUD_PROJECT`
   - Konum (isteğe bağlı, bu sırayla kontrol edilir):
     - `GOOGLE_VERTEX_LOCATION`
     - `GOOGLE_CLOUD_LOCATION`
     - `VERTEX_LOCATION`
     - Hiçbiri ayarlanmazsa varsayılan `us-central1`
