...
4. ป้อน Cloudflare API Token ของคุณ
   ...
   `CF_AIG_TOKEN` ยังสามารถใช้เป็นชื่อแทนสำหรับ `CLOUDFLARE_API_TOKEN` ได้อีกด้วย
...
2. ตั้งค่าตัวแปรสภาพแวดล้อมที่ต้องการ:
   - รหัสโครงการ (เลือกหนึ่งรายการ):
     - `GOOGLE_CLOUD_PROJECT`
     - `GCP_PROJECT`
     - `GCLOUD_PROJECT`
   - ตำแหน่ง (เป็นทางเลือก ตรวจสอบตามลำดับนี้):
     - `GOOGLE_VERTEX_LOCATION`
     - `GOOGLE_CLOUD_LOCATION`
     - `VERTEX_LOCATION`
     - ค่าเริ่มต้นคือ `us-central1` หากไม่ได้ตั้งค่าใดๆ
...
:::tip
ภูมิภาค `global` ปรับปรุงความพร้อมใช้งานและลดข้อผิดพลาดโดยไม่มีค่าใช้จ่ายเพิ่มเติม ใช้โดยตั้งค่า `VERTEX_LOCATION=global` ใช้ตำแหน่งข้อมูลระดับภูมิภาค (เช่น `us-central1`) สำหรับข้อกำหนดด้านถิ่นที่อยู่ของข้อมูล [เรียนรู้เพิ่มเติม](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#regional_and_global_endpoints)
:::
