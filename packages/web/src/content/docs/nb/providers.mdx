...
4. Skriv inn Cloudflare API-tokenet ditt.
   ...
   `CF_AIG_TOKEN` godtas også som et alias for `CLOUDFLARE_API_TOKEN`.
...
2. Angi de nødvendige miljøvariablene:
   - Prosjekt-ID (velg en):
     ...
     - `GCP_PROJECT`
     - `GCLOUD_PROJECT`
   - Plassering (valgfritt, sjekkes i denne rekkefølgen):
     - `GOOGLE_VERTEX_LOCATION`
     - `GOOGLE_CLOUD_LOCATION`
     - `VERTEX_LOCATION`
     - Standard er `us-central1` hvis ingen er satt
...
:::tip
`global`-regionen forbedrer tilgjengeligheten og reduserer feil uten ekstra kostnad. Bruk den ved å sette `VERTEX_LOCATION=global`. Bruk regionale endepunkter (f.eks. `us-central1`) for krav til dataopphold. [Finn ut mer](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#regional_and_global_endpoints)
:::
