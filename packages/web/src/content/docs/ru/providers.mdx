...
4. Введите свой токен API Cloudflare.
   ...
   `CF_AIG_TOKEN` также принимается как псевдоним для `CLOUDFLARE_API_TOKEN`.
...
2. Установите необходимые переменные среды:
   - Идентификатор проекта (выберите один):
     - `GOOGLE_CLOUD_PROJECT`
     - `GCP_PROJECT`
     - `GCLOUD_PROJECT`
   - Расположение (необязательно, проверяется в следующем порядке):
     - `GOOGLE_VERTEX_LOCATION`
     - `GOOGLE_CLOUD_LOCATION`
     - `VERTEX_LOCATION`
     - По умолчанию `us-central1`, если ничего не установлено
...
:::tip
Регион `global` повышает доступность и уменьшает количество ошибок без дополнительных затрат. Используйте его, установив `VERTEX_LOCATION=global`. Используйте региональные конечные точки (например, `us-central1`) для требований к местонахождению данных. [Подробнее](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#regional_and_global_endpoints)
:::
