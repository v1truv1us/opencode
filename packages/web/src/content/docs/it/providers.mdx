...
4. Inserisci il tuo API token di Cloudflare.
   ...
   `CF_AIG_TOKEN` è accettato anche come alias per `CLOUDFLARE_API_TOKEN`.
...
2. Imposta le variabili d'ambiente richieste:
   - ID progetto (scegline uno):
     - `GOOGLE_CLOUD_PROJECT`
     - `GCP_PROJECT`
     - `GCLOUD_PROJECT`
   - Posizione (opzionale, controllata in questo ordine):
     - `GOOGLE_VERTEX_LOCATION`
     - `GOOGLE_CLOUD_LOCATION`
     - `VERTEX_LOCATION`
     - Predefinito `us-central1` se nessuno è impostato
...
:::tip
La regione `global` migliora la disponibilità e riduce gli errori senza costi extra. Usala impostando `VERTEX_LOCATION=global`. Usa endpoint regionali (ad es. `us-central1`) per requisiti di residenza dei dati. [Scopri di più](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#regional_and_global_endpoints)
:::
