...
4. Indtast din Cloudflare API-token.
   ...
   `CF_AIG_TOKEN` accepteres også som et alias for `CLOUDFLARE_API_TOKEN`.
...
2. Angiv de nødvendige miljøvariabler:
   - Projekt-ID (vælg en):
     ...
     - `GCP_PROJECT`
     - `GCLOUD_PROJECT`
   - Lokation (valgfrit, kontrolleres i denne rækkefølge):
     - `GOOGLE_VERTEX_LOCATION`
     - `GOOGLE_CLOUD_LOCATION`
     - `VERTEX_LOCATION`
     - Standard er `us-central1`, hvis ingen er sat
   - Godkendelse (vælg en):
...
:::tip
`global`-regionen forbedrer tilgængeligheden og reducerer fejl uden ekstra omkostninger. Brug den ved at sætte `VERTEX_LOCATION=global`. Brug regionale endpoints (f.eks. `us-central1`) for krav til dataophold. [Læs mere](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#regional_and_global_endpoints)
:::
