...
`CF_AIG_TOKEN` je takođe prihvaćen kao alias za `CLOUDFLARE_API_TOKEN`.
...
2. Postavite potrebne varijable okruženja:
   - ID projekta (odaberite jedan):
     - `GOOGLE_CLOUD_PROJECT`
     - `GCP_PROJECT`
     - `GCLOUD_PROJECT`
   - Lokacija (opciono, provjerava se ovim redoslijedom):
     - `GOOGLE_VERTEX_LOCATION`
     - `GOOGLE_CLOUD_LOCATION`
     - `VERTEX_LOCATION`
     - Podrazumevano `us-central1` ako nijedno nije postavljeno
...
:::tip
Regija `global` poboljšava dostupnost i smanjuje greške bez dodatnih troškova. Koristite je postavljanjem `VERTEX_LOCATION=global`. Koristite regionalne krajnje tačke (npr. `us-central1`) za zahtjeve rezidentnosti podataka. [Saznajte više](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#regional_and_global_endpoints)
:::
