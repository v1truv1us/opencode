...
`CF_AIG_TOKEN` は `CLOUDFLARE_API_TOKEN` のエイリアスとしても使用できます。
...
2. 必要な環境変数を設定します。
   - プロジェクト ID（いずれか1つ）:
     ...
     - `GCP_PROJECT`
     - `GCLOUD_PROJECT`
   - ロケーション（オプション、この順序で確認）:
     - `GOOGLE_VERTEX_LOCATION`
     - `GOOGLE_CLOUD_LOCATION`
     - `VERTEX_LOCATION`
     - いずれも設定されていない場合のデフォルトは `us-central1`
   - 認証 (1 つ選択):
...
:::tip
`global` 領域は、追加コストなしで可用性を向上させ、エラーを削減します。`VERTEX_LOCATION=global` を設定して使用します。データ常駐要件には、リージョンエンドポイント (`us-central1` など) を使用します。 [詳細はこちら](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#regional_and_global_endpoints)
:::
