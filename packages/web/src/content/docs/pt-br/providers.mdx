...
4. Insira seu token da API do Cloudflare.
   ...
   `CF_AIG_TOKEN` também é aceito como alias para `CLOUDFLARE_API_TOKEN`.
...
2. Defina as variáveis de ambiente necessárias:
   - ID do projeto (escolha um):
     - `GOOGLE_CLOUD_PROJECT`
     - `GCP_PROJECT`
     - `GCLOUD_PROJECT`
   - Localização (opcional, verificado nesta ordem):
     - `GOOGLE_VERTEX_LOCATION`
     - `GOOGLE_CLOUD_LOCATION`
     - `VERTEX_LOCATION`
     - Padrão é `us-central1` se nenhum for definido
...
:::tip
A região `global` melhora a disponibilidade e reduz erros sem custo adicional. Use-a definindo `VERTEX_LOCATION=global`. Use endpoints regionais (ex: `us-central1`) para requisitos de residência de dados. [Saiba mais](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#regional_and_global_endpoints)
:::
